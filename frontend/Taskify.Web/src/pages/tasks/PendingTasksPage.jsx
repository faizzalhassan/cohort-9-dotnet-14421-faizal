...
function StatusBadgesRow({ tasks }) {
  ...
  return (
    <div className="flex flex-wrap items-center gap-2">
      <div className="inline-flex items-center gap-1.5 rounded-lg px-3 py-1.5 text-white shadow-md transition-all duration-300 hover:shadow-lg hover:scale-105 cursor-default bg-amber-600">
        <TIcon name="clock-pause" size={14} />
...
/* ─── Edit Drawer ────────────────────────────────────────────────────────── */
function EditDrawer({ isOpen, onClose, task, onSave }) {
      ...
      <div className={`fixed inset-y-0 right-0 z-[9999] flex flex-col w-full max-w-[440px] bg-white shadow-2xl transition-all duration-300 ease-[cubic-bezier(0.22,1,0.36,1)] ${isVisible ? 'translate-x-0 opacity-100' : 'translate-x-full opacity-0'}`}>
        ...
        <div className="flex-1 overflow-y-auto px-5 py-5 space-y-4">
          ...
          <div className="space-y-1.5">
            ...
            <input name="title" type="text" value={form.title} onChange={handleChange} placeholder="What needs to be done?" className={`${inputBase} ${errors.title ? inputError : inputNormal}`} />
          ...
          <div className="space-y-1.5">
            ...
            <textarea name="description" value={form.description} onChange={handleChange} placeholder="Add details (optional)" rows={3} className={`${inputBase} resize-none ${inputNormal}`} />
          </div>
          ...
          <div className="grid grid-cols-2 gap-3">
            <div className="space-y-1.5">
              ...
              <div className="relative">
                <select name="category" value={form.category} onChange={handleChange} className={`${inputBase} appearance-none pr-10 ${errors.category ? inputError : inputNormal}`}>
            ...
            <div className="space-y-1.5">
              ...
              <div className="relative">
                <select name="priority" value={form.priority} onChange={handleChange} className={`${inputBase} appearance-none pr-10 ${inputNormal}`}>
          ...
          <div className="space-y-1.5">
            ...
            <input name="dueDate" type="date" value={form.dueDate} onChange={handleChange} className={`${inputBase} ${errors.dueDate ? inputError : inputNormal}`} />
        ...
        <div className="shrink-0 border-t border-slate-100 px-5 py-4 bg-white">
          <div className="flex items-center justify-end gap-2.5">
            <button type="button" onClick={handleClose} disabled={isSubmitting} className="rounded-xl px-5 py-2.5 text-[13px] font-medium text-slate-500 hover:bg-slate-100 disabled:opacity-50 transition-all duration-200">
              Cancel
            </button>
            <button onClick={handleSubmit} disabled={isSubmitting} className="flex items-center gap-2 rounded-xl bg-slate-900 px-5 py-2.5 text-[13px] font-medium text-white hover:bg-slate-800 hover:shadow-lg active:scale-[0.97] disabled:opacity-50 transition-all duration-200">
              {isSubmitting
...
/* ─── Premium View Drawer ───────────────────────────────────────────────── */
function ViewDrawer({ isOpen, onClose, task, onStatusChange, canChangeStatus, isOwnTask, onEdit, onDelete }) {
  const [isVisible, setIsVisible] = useState(false);
  ...
  return (
    <>
      ...
      <div className={`fixed inset-y-0 right-0 z-[9999] flex flex-col w-full max-w-[440px] bg-white shadow-2xl transition-all duration-300 ease-[cubic-bezier(0.22,1,0.36,1)] ${isVisible ? 'translate-x-0 opacity-100' : 'translate-x-full opacity-0'}`}>
        ...
        {/* Header - Simplified */}
        <div style={{ borderTop: `3px solid ${catStyle.border}` }} className="shrink-0">
          <div className="flex items-center justify-between px-5 py-4 border-b border-slate-100">
            <h2 className="text-[16px] font-semibold text-slate-900">{task.title}</h2>
            <button onClick={onClose} className="grid h-8 w-8 place-items-center rounded-lg text-slate-400 hover:bg-slate-100 hover:text-slate-600 transition-all duration-200">
              <TIcon name="x" size={18} />
            </button>
          </div>
        </div>
    ...
    <div className="space-y-6">
      ...
      {tasks.length === 0 ? (
        <div className="flex flex-col items-center justify-center rounded-2xl border border-slate-200 bg-white py-16 shadow-sm">
          ...
          <p className="mt-4 text-[16px] font-medium text-slate-700">No pending tasks</p>
          <p className="mt-1 text-[13px] text-slate-400">All caught up!</p>
        </div>
      ) : (
        <TableView
          tasks={tasks}
