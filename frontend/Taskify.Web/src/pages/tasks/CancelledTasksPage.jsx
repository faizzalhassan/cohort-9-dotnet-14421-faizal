...
function StatusBadgesRow({ tasks }) {
  const cancelledCount = tasks.length;

  return (
    <div className="flex flex-wrap items-center gap-2">
      <div className="inline-flex items-center gap-1.5 rounded-lg px-3 py-1.5 text-white shadow-md transition-all duration-300 hover:shadow-lg hover:scale-105 cursor-default bg-slate-600">
        <TIcon name="circle-x" size={14} />
...
export default function CancelledTasksPage() {
  ...
  const fetchTasks = useCallback(async () => {
    ...
    try {
      const response = await taskService.getCancelledTasks({});
  ...
  const handleStatusChange = async (taskId, newStatus) => {
    ...
    if (response.success) {
      // Remove task from this page since it's no longer Cancelled
    ...
    <div className="space-y-6">
      ...
      {tasks.length === 0 ? (
        <div className="flex flex-col items-center justify-center rounded-2xl border border-slate-200 bg-white py-16 shadow-sm">
          ...
          <p className="mt-4 text-[16px] font-medium text-slate-700">No cancelled tasks</p>
          <p className="mt-1 text-[13px] text-slate-400">Cancelled tasks will appear here</p>
        </div>
      ) : (
        <TableView
          tasks={tasks}
