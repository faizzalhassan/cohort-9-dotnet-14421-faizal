...
/* ─── Premium View Drawer ───────────────────────────────────────────────── */
function ViewDrawer({ isOpen, onClose, task, onStatusChange, canChangeStatus }) {
      ...
      <div className={`fixed inset-y-0 right-0 z-[9999] flex flex-col w-full max-w-[440px] bg-white shadow-2xl transition-all duration-300 ease-[cubic-bezier(0.22,1,0.36,1)] ${isVisible ? 'translate-x-0 opacity-100' : 'translate-x-full opacity-0'}`}>
        ...
        <div className="flex-1 overflow-y-auto px-5 py-5">
          ...
          <div className="py-4">
            ...
            <div className="space-y-2">
              <div className={`inline-flex items-center gap-2 mr-2 rounded-xl px-3 py-2 ${isOverdue ? 'bg-red-50 border border-red-200' : 'bg-slate-50 border border-slate-200'}`}>
        ...
        <div className="shrink-0 border-t border-slate-100 px-5 py-4 bg-white">
          {canChangeStatus ? (
            ...
            <button
              onClick={onClose}
              className="w-full rounded-xl bg-slate-900 px-6 py-2.5 text-[13px] font-medium text-white hover:bg-slate-800 hover:shadow-lg active:scale-[0.97] transition-all duration-200"
            >
              Close
            </button>
          )}
  ...
  const fetchTasks = useCallback(async () => {
    ...
    try {
      ...
      if (response.success) {
        ...
        setError(response.message || 'Failed to load assigned tasks');
      }
    } catch (err) {
    ...
    <div className="space-y-6">
      ...
      {tasks.length === 0 ? (
        <div className="flex flex-col items-center justify-center rounded-2xl border border-slate-200 bg-white py-16 shadow-sm">
          ...
          <p className="mt-4 text-[16px] font-medium text-slate-700">No assigned tasks</p>
          <p className="mt-1 text-[13px] text-slate-400">You have no tasks assigned to you</p>
        </div>
      ) : (
        <TableView
          tasks={tasks}
