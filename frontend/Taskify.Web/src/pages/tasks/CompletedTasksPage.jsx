...
function StatusBadgesRow({ tasks }) {
  ...
  return (
    <div className="flex flex-wrap items-center gap-2">
      <div className="inline-flex items-center gap-1.5 rounded-lg px-3 py-1.5 text-white shadow-md transition-all duration-300 hover:shadow-lg hover:scale-105 cursor-default bg-emerald-600">
        <TIcon name="circle-check" size={14} />
...
export default function CompletedTasksPage() {
  ...
  const handleStatusChange = async (taskId, newStatus) => {
    ...
    if (response.success) {
      // Remove task from this page since it's no longer Completed
    ...
    <div className="space-y-6">
      ...
      {tasks.length === 0 ? (
        <div className="flex flex-col items-center justify-center rounded-2xl border border-slate-200 bg-white py-16 shadow-sm">
          ...
          <p className="mt-4 text-[16px] font-medium text-slate-700">No completed tasks</p>
          <p className="mt-1 text-[13px] text-slate-400">Completed tasks will appear here</p>
        </div>
      ) : (
        <TableView
          tasks={tasks}
