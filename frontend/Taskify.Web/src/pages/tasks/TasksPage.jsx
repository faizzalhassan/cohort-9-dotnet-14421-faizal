...
function getDaysUntilDue(task) {
  ...
  const dueDateKey = getDateKey(task.dueDate);
  const todayKey = getTodayKey();
  if (!dueDateKey || dueDateKey < todayKey) return null;
...
const taskStatusOptions = [
  { value: 'Pending', label: 'Pending', icon: 'clock-pause', bg: 'bg-amber-50', text: 'text-amber-700' },
  { value: 'InProgress', label: 'In Progress', icon: 'progress', bg: 'bg-indigo-50', text: 'text-indigo-700' },
  { value: 'Completed', label: 'Completed', icon: 'circle-check', bg: 'bg-emerald-50', text: 'text-emerald-700' },
  { value: 'Cancelled', label: 'Cancelled', icon: 'circle-x', bg: 'bg-slate-100', text: 'text-slate-600' },
];
...
const categoryColors = {
  Work: { badge: 'bg-indigo-100 text-indigo-700', border: '#6366f1', viewBg: 'bg-indigo-50', viewText: 'text-indigo-700' },
  Personal: { badge: 'bg-emerald-100 text-emerald-700', border: '#10b981', viewBg: 'bg-emerald-50', viewText: 'text-emerald-700' },
  Study: { badge: 'bg-purple-100 text-purple-700', border: '#a855f7', viewBg: 'bg-purple-50', viewText: 'text-purple-700' },
  Meetings: { badge: 'bg-amber-100 text-amber-700', border: '#f59e0b', viewBg: 'bg-amber-50', viewText: 'text-amber-700' },
  Finance: { badge: 'bg-green-100 text-green-700', border: '#22c55e', viewBg: 'bg-green-50', viewText: 'text-green-700' },
  Shopping: { badge: 'bg-pink-100 text-pink-700', border: '#ec4899', viewBg: 'bg-pink-50', viewText: 'text-pink-700' },
  Health: { badge: 'bg-red-100 text-red-700', border: '#ef4444', viewBg: 'bg-red-50', viewText: 'text-red-700' },
  Home: { badge: 'bg-orange-100 text-orange-700', border: '#f97316', viewBg: 'bg-orange-50', viewText: 'text-orange-700' },
  Travel: { badge: 'bg-cyan-100 text-cyan-700', border: '#06b6d4', viewBg: 'bg-cyan-50', viewText: 'text-cyan-700' },
  Other: { badge: 'bg-slate-100 text-slate-700', border: '#94a3b8', viewBg: 'bg-slate-100', viewText: 'text-slate-600' },
};
  ...
  return (
    <div className="flex flex-wrap gap-1.5 mt-1.5">
      {QUICK_DATE_OPTIONS.map((option) => {
        ...
        const selected = currentDate === dateKey;
        return (
          <button
            key={option.days}
            type="button"
...
function ViewEditDrawer({
  ...
  const validateForm = () => {
    ...
    if (isAdmin && !form.assignedToUserId) newErrors.assignedToUserId = 'Please assign a user';
    setErrors(newErrors);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const detailsChanged =
        ...
        (form.description.trim() || '') !== (task?.description || '').trim() ||
        form.category !== (task?.category || '') ||
        form.priority !== (task?.priority || 'Medium') ||
        form.dueDate !== (task?.dueDate ? task.dueDate.split('T')[0] : '') ||
        (isAdmin && String(form.assignedToUserId || '') !== String(task?.assignedToUserId || ''));
      ...
      if (form.status !== originalStatus) {
        const statusResponse = await taskService.updateTaskStatus(task.id, form.status);
        if (!statusResponse.success) {
          setServerError(statusResponse.message || 'Failed to update task status');
          return;
        }
        savedTask = {
          ...(savedTask || task),
  ...
  const getHeaderInfo = () => {
    if (isViewMode) {
      return { title: 'Task Details', subtitle: 'Read-only view', icon: 'eye', iconBg: 'bg-blue-50', iconColor: 'text-blue-600' };
    }
    return { title: 'Edit Task', subtitle: 'Update task details', icon: 'pencil', iconBg: 'bg-amber-50', iconColor: 'text-amber-600' };
  };
  ...
  return (
    <>
      ...
      >
        <div
          style={{ borderTop: isViewMode ? `3px solid ${catStyle.border}` : '3px solid #1e293b' }}
          className="shrink-0"
        >
          <div className="flex items-center justify-between px-5 py-4 border-b border-slate-100">
            <div className="flex items-center gap-3">
              <div className={`grid h-9 w-9 place-items-center rounded-xl ${headerInfo.iconBg}`}>
                <TIcon name={headerInfo.icon} size={18} className={headerInfo.iconColor} />
              </div>
              <div>
                <h2 className="text-[15px] font-semibold text-slate-900 leading-tight">{headerInfo.title}</h2>
                <p className="text-[11px] text-slate-400 mt-0.5">{headerInfo.subtitle}</p>
              </div>
            </div>
            <button
              onClick={handleClose}
          ...
          {isViewMode ? (
            <>
              <div className="pb-4 border-b border-slate-100">
                <p className="text-[10px] font-semibold uppercase tracking-widest text-slate-400 mb-1.5">Title</p>
                <h3 className="text-[17px] font-bold text-slate-900 leading-snug">{task.title}</h3>
                {task.description ? (
                  <p className="mt-2 text-[13px] text-slate-500 leading-relaxed">{task.description}</p>
                ) : (
                ...
                <div className="grid grid-cols-3 gap-3">
                  ...
                  <div className="rounded-xl border border-slate-100 bg-slate-50 p-3 flex flex-col gap-1.5">
                    ...
                    <div className="relative">
                      ...
                      <TIcon name="chevron-down" size={12} className="pointer-events-none absolute right-2 top-1/2 -translate-y-1/2 text-slate-400" />
                    </div>
                  </div>
                </div>
              </div>
              ...
              <div className="py-4 border-b border-slate-100">
                ...
                <div className={`inline-flex items-center gap-2 rounded-xl px-3 py-2 ${isOverdue ? 'bg-red-50 border border-red-200' : 'bg-slate-50 border border-slate-200'}`}>
              ...
              <div className="py-4 border-b border-slate-100">
                <p className="text-[10px] font-semibold uppercase tracking-widest text-slate-400 mb-2.5">People</p>
                <div className="space-y-2">
                  <div className="flex items-center gap-3 rounded-xl bg-slate-50 border border-slate-100 px-3 py-2.5">
                    <div className="grid h-7 w-7 shrink-0 place-items-center rounded-lg bg-white border border-slate-200">
                      <TIcon name="user" size={13} className="text-slate-400" />
                    </div>
                    <div>
                      <p className="text-[10px] text-slate-400 font-medium">Created by</p>
                      <p className="text-[13px] font-semibold text-slate-700">{task.createdByName || 'Unknown'}</p>
                    </div>
                  </div>
                  {task.assignedToName && (
                    <div className="flex items-center gap-3 rounded-xl bg-slate-50 border border-slate-100 px-3 py-2.5">
                      <div className="grid h-7 w-7 shrink-0 place-items-center rounded-lg bg-white border border-slate-200">
                        <TIcon name="user-check" size={13} className="text-slate-400" />
                      </div>
                      <div>
                        <p className="text-[10px] text-slate-400 font-medium">Assigned to</p>
                        <p className="text-[13px] font-semibold text-slate-700">{task.assignedToName}</p>
                      </div>
                    </div>
                  )}
                </div>
            ...
            <form className="space-y-4">
              <div className="space-y-1.5">
                ...
                {errors.title && (
                  <p className="flex items-center gap-1 text-[11px] text-red-500">
                    <TIcon name="alert-circle" size={12} />{errors.title}
                  </p>
                )}
              </div>
                ...
                <div className="space-y-1.5">
                  ...
                  <div className="relative">
                    <select
                      ...
                      value={form.category}
                      onChange={handleChange}
                      className={`${inputBase} appearance-none pr-10 ${errors.category ? inputError : inputNormal}`}
                    >
                  ...
                  {errors.category && (
                    <p className="flex items-center gap-1 text-[11px] text-red-500">
                      <TIcon name="alert-circle" size={12} />{errors.category}
                    </p>
                  )}
                </div>
              ...
              <div className="space-y-1.5">
                <label className="block text-[12px] font-medium text-slate-600">Status</label>
                <div className="relative">
                  <select
                    name="status"
                    value={form.status}
              ...
              <div className="space-y-1.5">
                ...
                />
                <QuickDateButtons onSelect={handleQuickDateSelect} currentDate={form.dueDate} />
                {errors.dueDate && (
                  <p className="flex items-center gap-1 text-[11px] text-red-500">
                    <TIcon name="alert-circle" size={12} />{errors.dueDate}
                  </p>
                )}
              </div>
                ...
                <div className="space-y-1.5">
                  ...
                  <div className="relative">
                    <select
                      ...
                      onChange={handleChange}
                      className={`${inputBase} appearance-none pr-10 ${errors.assignedToUserId ? inputError : inputNormal}`}
                    >
                  ...
                  {errors.assignedToUserId && (
                    <p className="flex items-center gap-1 text-[11px] text-red-500">
                      <TIcon name="alert-circle" size={12} />{errors.assignedToUserId}
                    </p>
                  )}
                </div>
              )}
              ...
              {canEdit && (
                <button
                  onClick={() => {
                    ...
                    if (editHandler) editHandler(task);
                  }}
...
/* ─── Edit/Create Drawer ─────────────────────────────────────────────────── */
function EditDrawer({ isOpen, onClose, task, mode, onSave, isAdmin }) {
  const [form, setForm] = useState({
    title: '',
    description: '',
    category: '',
  ...
  useEffect(() => {
    if (task && mode === 'edit') {
      ...
      setForm({ title: '', description: '', category: '', priority: 'Medium', dueDate: '', assignedToUserId: '', status: 'Pending' });
    }
    setErrors({});
    setServerError('');
  }, [task, mode, isOpen]);
    ...
    try {
      ...
      if (mode === 'edit') {
        ...
        if (detailsChanged) {
          const response = await taskService.updateTask(task.id, data);
          if (!response.success) {
            setServerError(response.message || 'Failed to update task');
            return;
          }
          savedTask = response.data;
        }
        ...
        if (response.success) {
          ...
          setServerError(response.message || 'Failed to create task');
        }
      }
    } catch (err) {
      setServerError(
    ...
    <>
      ...
      >
        <div className="shrink-0 flex items-center justify-between border-b border-slate-100 px-5 py-4">
          <div className="flex items-center gap-3">
            <div className={`grid h-9 w-9 place-items-center rounded-xl ${mode === 'edit' ? 'bg-amber-50' : 'bg-emerald-50'}`}>
              <TIcon
                name={mode === 'edit' ? 'pencil' : 'plus'}
                size={18}
                className={mode === 'edit' ? 'text-amber-600' : 'text-emerald-600'}
              />
            </div>
          ...
          <div className="flex items-center justify-end gap-2.5">
            ...
            >
              {isSubmitting ? (
                ...
                <>
                  <TIcon name={mode === 'edit' ? 'check' : 'plus'} size={16} />
...
function KanbanCard({ task, onView }) {
  ...
  const formatCreatedDate = (d) => {
    ...
    const diffDays = Math.floor((startOfToday.getTime() - createdDay.getTime()) / (1000 * 60 * 60 * 24));
  ...
  return (
    <div
      ...
      style={{ borderLeft: `3px solid ${catStyle.border}` }}
      onClick={() => onView(task)}
    >
      <div className="flex items-start justify-between gap-1.5 mb-1">
        <div className="flex-1 min-w-0">
          {task.category && (
            <span className={`inline-flex items-center gap-1 rounded-md px-1.5 py-0.5 text-[10px] font-medium mb-1 ${catStyle.badge}`}>
              <TIcon name="folder" size={10} />
              {task.category}
            </span>
          )}
          <h4 className="text-[13px] font-semibold text-slate-900 leading-snug line-clamp-2">{task.title}</h4>
        </div>
        <div className="shrink-0 mt-0.5 opacity-0 group-hover:opacity-100 transition-opacity duration-200">
          <TIcon name="chevron-right" size={16} className="text-slate-400" />
        </div>
      </div>

      <div className="mb-2">
        {hasDescription ? (
          <p className="text-[12px] text-slate-500 line-clamp-2 leading-snug">{task.description}</p>
        ) : (
      ...
      <div className="flex items-center justify-between gap-2 mb-1.5">
        ...
        {isOverdue ? (
          ...
          <span className={`text-[10px] font-medium ${daysLeft <= 2 ? 'text-red-600' : daysLeft <= 5 ? 'text-amber-600' : 'text-slate-500'}`}>
      ...
      <div className="mt-1.5 pt-1.5 border-t border-slate-100 flex items-center justify-between text-[10px] text-slate-400">
        <div className="flex items-center gap-1">
          ...
          <span className="italic">{formatCreatedDate(task.createdAt)}</span>
        </div>
        <div className="flex items-center gap-1">
          <TIcon name="calendar" size={10} />
          <span className="italic">{formatDate(task.dueDate)}</span>
        </div>
      </div>
    </div>
  );
...
/* ─── Kanban Column ──────────────────────────────────────────────────────── */
function KanbanColumn({ title, icon, headerColor, headerBg, tasks, onView }) {
  return (
    <div className="flex flex-col flex-1 min-w-[220px]">
      <div className={`flex items-center gap-1.5 rounded-lg px-2.5 py-2 mb-2.5 ${headerBg}`}>
        <TIcon name={icon} size={14} className={headerColor} />
        <span className={`text-[11px] font-semibold uppercase tracking-wide ${headerColor}`}>{title}</span>
      </div>
      <div className="flex flex-col gap-2 flex-1 pr-0.5" style={{ overflowX: 'visible' }}>
        {tasks.length === 0 ? (
          <div className="rounded-lg border border-dashed border-slate-200 bg-slate-50/50 py-6 flex items-center justify-center">
            <span className="text-[11px] text-slate-400">No tasks</span>
          </div>
        ) : (
          tasks.map((task) => (
...
function KanbanView({ tasks, onView }) {
  const columns = [
    { key: 'Pending', title: 'Pending', icon: 'clock-pause', headerColor: 'text-amber-700', headerBg: 'bg-amber-50' },
    { key: 'InProgress', title: 'In Progress', icon: 'progress', headerColor: 'text-indigo-700', headerBg: 'bg-indigo-50' },
    { key: 'Completed', title: 'Completed', icon: 'circle-check', headerColor: 'text-emerald-700', headerBg: 'bg-emerald-50' },
    { key: 'Cancelled', title: 'Cancelled', icon: 'circle-x', headerColor: 'text-slate-600', headerBg: 'bg-slate-100' },
    { key: 'Overdue', title: 'Overdue', icon: 'alert-triangle', headerColor: 'text-red-700', headerBg: 'bg-red-50' },
  ];

  const grouped = columns.reduce((acc, col) => {
    if (col.key === 'Overdue') {
      acc[col.key] = tasks.filter((task) => isTaskOverdue(task));
    } else {
      acc[col.key] = tasks.filter(
        (task) =>
          !isTaskOverdue(task) &&
          (task.status === col.key || task.status?.toLowerCase() === col.key.toLowerCase())
      );
    }
    return acc;
  }, {});
  ...
  return (
    <div className="flex gap-3" style={{ overflowX: 'auto', overflowY: 'visible', paddingBottom: 4 }}>
      {columns.map((col) => (
        <KanbanColumn
          key={col.key}
          title={col.title}
...
function FilterSelect({ icon, value, onChange, options }) {
  return (
    <div className="relative inline-flex items-center flex-1 min-w-[90px]">
      {icon && (
        <TIcon name={icon} size={14} className="absolute left-2.5 text-slate-400 pointer-events-none z-10" />
      )}
      <select
        value={value}
      ...
      <TIcon name="chevron-down" size={13} className="absolute right-2.5 text-slate-400 pointer-events-none" />
    </div>
  );
}
...
export default function TasksPage() {
  ...
  const [viewEditDrawerOpen, setViewEditDrawerOpen] = useState(false);
  ...
  /* ─── SignalR: register listener on the shared connection ─────────────── */
  useEffect(() => {
    let isMounted = true;
    ...
    const handleTaskAssigned = (incomingTask) => {
      console.info('SignalR TasksPage: TaskAssigned received:', incomingTask);
      ...
      setTasks((prevTasks) => {
        ...
        if (exists) {
          return prevTasks.map((t) => (t.id === incomingTask.id ? incomingTask : t));
        }
    ...
    const registerListener = async () => {
      try {
        ...
        console.error('SignalR TasksPage: failed to register listener:', err);
      }
    };
  ...
  const filteredTasks = useMemo(() => {
    ...
    const endOfWeekKey = addDaysToDateKey(todayKey, daysUntilSaturday);
    ...
    if (dueFilter !== 'all') {
      result = result.filter((task) => {
        ...
        switch (dueFilter) {
          ...
          case 'today': return dueDateKey === todayKey;
          case 'tomorrow': return dueDateKey === tomorrowKey;
          case 'thisWeek': return dueDateKey >= todayKey && dueDateKey <= endOfWeekKey;
          default: return true;
        }
      });
    }
    ...
    if (taskTypeFilter === 'myTasks') {
      result = result.filter((task) => task.createdByUserId === user?.id);
    } else if (taskTypeFilter === 'assignedToMe') {
      result = result.filter((task) => task.assignedToUserId === user?.id);
    }
    ...
    switch (sortBy) {
      ...
      case 'dueEarliest':
        result.sort((a, b) => {
          ...
          return getDateKey(a.dueDate).localeCompare(getDateKey(b.dueDate));
        });
        break;
      case 'dueLatest':
        result.sort((a, b) => {
          if (!a.dueDate) return 1;
          if (!b.dueDate) return -1;
          return getDateKey(b.dueDate).localeCompare(getDateKey(a.dueDate));
        });
        break;
      ...
      case 'titleZA': result.sort((a, b) => b.title.localeCompare(a.title)); break;
      default: break;
    }

    return result;
  }, [tasks, dueFilter, priorityFilter, taskTypeFilter, sortBy, user]);

  const isFilterActive = dueFilter !== 'all' || priorityFilter !== 'all' || taskTypeFilter !== 'all' || sortBy !== 'newest';
  ...
  const handleStatusChange = async (task, newStatus) => {
    ...
    try {
      ...
      setActiveTask((prev) => (prev && prev.id === task.id ? updatedTask : prev));
    } catch (err) {
      setError(
        err?.response?.data?.message ||
        err?.response?.data?.title ||
  ...
  useEffect(() => {
    if (viewEditDrawerOpen && drawerMode === 'view') {
      window.__switchToEditMode = (task) => { handleEditTask(task); };
    }
    return () => { delete window.__switchToEditMode; };
  ...
  const canEdit = (task) => isAdmin || task?.createdByUserId === user?.id;
  const canDelete = (task) => isAdmin || task?.createdByUserId === user?.id;

  /* ─── Static config ────────────────────────────────────────────────────── */
  const statusBadges = [
    { key: 'Pending', label: 'Pending', icon: 'clock-pause', color: 'bg-amber-600 hover:bg-amber-700' },
    { key: 'InProgress', label: 'In Progress', icon: 'progress', color: 'bg-blue-600 hover:bg-blue-700' },
    { key: 'Completed', label: 'Completed', icon: 'circle-check', color: 'bg-emerald-600 hover:bg-emerald-700' },
    { key: 'Cancelled', label: 'Cancelled', icon: 'circle-x', color: 'bg-slate-600 hover:bg-slate-700' },
    { key: 'Overdue', label: 'Overdue', icon: 'alert-triangle', color: 'bg-red-600 hover:bg-red-700' },
  ];
  ...
  const sortOptions = [
    ...
    { value: 'priorityLowest', label: 'Priority: Lowest' },
  ...
  const Toolbar = () => (
    <div className="flex flex-wrap items-center gap-2">
      ...
      <div className="flex flex-1 flex-wrap items-center gap-1.5 min-w-[200px]">
        <FilterSelect icon="calendar" value={dueFilter} onChange={setDueFilter} options={dueOptions} />
        <FilterSelect icon="flag" value={priorityFilter} onChange={setPriorityFilter} options={priorityOptions} />
        <FilterSelect icon="users" value={taskTypeFilter} onChange={setTaskTypeFilter} options={taskTypeOptions} />
        <FilterSelect icon="sort-descending" value={sortBy} onChange={setSortBy} options={sortOptions} />
        {isFilterActive && (
          <button
      ...
      >
        ...
        <span className="text-[12px] font-semibold">New Task</span>
      </button>
    </div>
  );
  ...
  const Overlays = () => (
    <>
      <ViewEditDrawer
        ...
        onClose={() => { setViewEditDrawerOpen(false); setActiveTask(null); }}
        task={activeTask}
        mode={drawerMode}
        ...
        canEdit={activeTask ? canEdit(activeTask) : false}
        canDelete={activeTask ? canDelete(activeTask) : false}
      />

      <EditDrawer
        isOpen={editDrawerOpen}
        onClose={() => { setEditDrawerOpen(false); setActiveTask(null); }}
        task={activeTask}
        mode="create"
        onSave={handleTaskSaved}
        isAdmin={isAdmin}
  ...
  if (loading) {
    return (
      <div className="flex flex-col items-center justify-center py-20">
        ...
        <p className="mt-4 text-[13px] text-slate-400 animate-pulse">Loading tasks...</p>
      </div>
    );
  }
      ...
      <div className="space-y-6">
        ...
        <div className="flex flex-col items-center justify-center rounded-2xl border border-slate-200 bg-white py-16 shadow-sm">
          <div className="grid h-20 w-20 place-items-center rounded-full bg-slate-50 ring-1 ring-slate-100">
            <TIcon name="filter-off" size={32} className="text-slate-400" />
          </div>
          <p className="mt-4 text-[16px] font-medium text-slate-700">No matching tasks</p>
          <p className="mt-1 text-[13px] text-slate-400">Try changing or clearing your filters.</p>
          <button
            onClick={resetFilters}
    ...
    <div className="space-y-6">
      ...
      {tasks.length === 0 ? (
        <div className="flex flex-col items-center justify-center rounded-2xl border border-slate-200 bg-white py-16 shadow-sm">
          ...
          <p className="mt-4 text-[16px] font-medium text-slate-700">No tasks yet</p>
          <p className="mt-1 text-[13px] text-slate-400">Create your first task to get started</p>
          <button
            onClick={handleCreateTask}
            className="mt-6 flex items-center gap-2 rounded-xl bg-slate-900 px-5 py-2.5 text-[13px] font-medium text-white hover:bg-slate-800 hover:shadow-lg active:scale-[0.97] transition-all duration-200"
          >
        ...
        <KanbanView tasks={filteredTasks} onView={handleViewTask} />
      )}
