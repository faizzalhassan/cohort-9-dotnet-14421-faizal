...
function StatusBadgesRow({ tasks }) {
  ...
  return (
    <div className="flex flex-wrap items-center gap-2">
      <div className="inline-flex items-center gap-1.5 rounded-lg px-3 py-1.5 text-white shadow-md transition-all duration-300 hover:shadow-lg hover:scale-105 cursor-default bg-red-600">
...
function ViewEditDrawer({ isOpen, onClose, task, onStatusChange, canChangeStatus, isOwnTask, onDelete, onSave }) {
  ...
  const isOverdue = true; // All tasks in this page are overdue
        ...
        <div className="flex-1 overflow-y-auto px-5 py-5">
          ...
          {mode === 'edit' ? (
            ...
            <>
              ...
              {/* Due Date with spacing - Overdue specific */}
              <div className="py-4">
                <p className="text-[10px] font-semibold uppercase tracking-widest text-slate-400 mb-2.5">Due Date</p>
                <div className="space-y-2">
                  <div className="inline-flex items-center gap-2 rounded-xl px-3 py-2 mr-2 bg-red-50 border border-red-200">
                    <TIcon name="calendar-x" size={15} className="text-red-500" />
                  ...
                  <td className="px-4 py-3">
                    <div className="flex flex-col">
                      ...
                      {daysOverdue !== null && daysOverdue > 0 && (
                        <span className="text-[10px] text-red-400 ml-5">
...
export default function OverdueTasksPage() {
  ...
  const handleStatusChange = async (taskId, newStatus) => {
    ...
    if (response.success) {
      // If status changes to Completed or Cancelled, remove from overdue
      ...
      {tasks.length === 0 ? (
        <div className="flex flex-col items-center justify-center rounded-2xl border border-slate-200 bg-white py-16 shadow-sm">
          <div className="grid h-20 w-20 place-items-center rounded-full bg-slate-50 ring-1 ring-slate-100">
            <TIcon name="circle-check" size={32} className="text-emerald-400" />
          </div>
          <p className="mt-4 text-[16px] font-medium text-slate-700">No overdue tasks</p>
          <p className="mt-1 text-[13px] text-slate-400">Everything is on track!</p>
        </div>
      ) : (
        <TableView
          tasks={tasks}
