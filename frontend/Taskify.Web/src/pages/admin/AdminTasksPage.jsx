...
function Avatar({ name, size = 'md' }) {
  ...
  return (
    <div
      className={`grid shrink-0 place-items-center rounded-full font-bold ${sz} ${bg} ${text}`}
    >
      {getInitials(name)}
    </div>
  );
...
function PriorityBadge({ priority }) {
  const priorityLabel =
    typeof priority === 'number'
      ? priorityEnumMap[priority] || 'Medium'
      : priority;

  const s =
    priorityConfig[priorityLabel] || priorityConfig.Medium;

  return (
    <span
      className={`inline-flex items-center gap-1 rounded-md px-2 py-0.5 text-[11px] font-medium ${s.bg} ${s.text}`}
    >
      <TIcon name={s.icon} size={11} />
      {priorityLabel}
    </span>
  );
}
...
function StatusBadge({ status }) {
  ...
  const s =
    statusConfig[statusLabel] || statusConfig.Pending;
...
function InlineStatusDropdown({
  ...
  return (
    ...
    >
      ...
      {isOpen && (
        <div
          className="absolute left-0 top-full z-50 mt-1 w-40 overflow-hidden rounded-xl border border-slate-200 bg-white shadow-xl"
...
function TaskDrawer({
  ...
  const handleSubmit = async () => {
    ...
    if (!formData.assignedToUserId) {
      setError(
        'Please assign a user to the task.'
      );
      return;
    }
    ...
    try {
      ...
      /*
       * Important:
       * After saving, close the entire drawer.
       */
      onClose();
    } catch (err) {
      setError(
  ...
  /* ─── Determine if task belongs to current day / overdue ─────────────── */
   ...
   * IMPORTANT LOGIC:
   *
   * If Created By and Assigned To are the same person,
   * this is a user-created personal task.
   *
   * Therefore:
   ...
   *   Created By → show admin
   *   Assigned To → show user
   */
  const hasDifferentCreatorAndAssignee =
    Boolean(
      task?.createdByName &&
        task?.assignedToName &&
        task.createdByName
            ...
            'view' ? (
              ...
              <div className="space-y-5">
                ...
                <div className="border-b border-slate-100 pb-4">
                  ...
                  <div className="grid grid-cols-3 gap-3">
                    <div className="flex flex-col gap-1.5 rounded-xl border border-slate-100 bg-slate-50 p-3">
                      ...
                      <span
                        className={`inline-flex items-center gap-1 self-start rounded-md px-2 py-1 text-[11px] font-semibold ${cat.badge}`}
                      >
                        <TIcon
                          name="folder"
                          size={11}
                    ...
                    <div className="flex flex-col gap-1.5 rounded-xl border border-slate-100 bg-slate-50 p-3">
                      ...
                      <span
                        className={`inline-flex items-center gap-1 self-start rounded-md px-2 py-1 text-[11px] font-semibold ${prio.bg} ${prio.text}`}
                      >
                        <TIcon
                          name={
                            prio.icon
                    ...
                    <div className="flex flex-col gap-1.5 rounded-xl border border-slate-100 bg-slate-50 p-3">
                      ...
                      <span
                        className={`inline-flex items-center gap-1 self-start rounded-md px-2 py-1 text-[11px] font-semibold ${stat.bg} ${stat.text}`}
                      >
                        <TIcon
                          name={
                            stat.icon
                ...
                {task?.createdByName && (
                  <div
                    className={
                      hasDifferentCreatorAndAssignee
                ...
                {/* Assigned To
                    ONLY shown when creator and assignee are different */}
                {hasDifferentCreatorAndAssignee && (
                  <div className="border-b border-slate-100 pb-4">
                    <p className="mb-2.5 text-[10px] font-semibold uppercase tracking-widest text-slate-400">
                      Assigned To
                ...
                <tbody className="divide-y divide-slate-100">
                  {filteredAndSorted.map(
                    (task) => {
                      ...
                      /*
                       * Same creator and assignee means:
                       * user created the task for themselves.
                       *
                       * Different names means:
                       * admin created task and assigned it to a user.
                       */
                      const isSamePerson =
                        task.createdByName &&
                        task.assignedToName &&
                      ...
                      return (
                        ...
                        >
                          ...
                          <td className="px-4 py-3">
                            <span
                              className={`inline-flex items-center gap-1 rounded-md px-2 py-0.5 text-[11px] font-medium ${cat.badge}`}
                            >
                              <TIcon
                                name="folder"
                                size={10}
