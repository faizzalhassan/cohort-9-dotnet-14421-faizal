...
public class TasksController : ControllerBase
{
    ...
    public async Task<IActionResult> GetCancelledTasks()
    {
        ...
        var tasks = await _taskService.GetCancelledTasksAsync(
            userId,
            userRole);
    ...
    public async Task<IActionResult> GetAdminTask(int id)
    {
        ...
        var task = await _taskService.GetAdminTaskByIdAsync(
            id,
            adminUserId,
            adminRole);
    ...
    [Authorize(Roles = "Admin")]
    public async Task<IActionResult> CreateAdminTask(
        [FromBody] CreateTaskRequest request)
    {
        var adminUserId = GetCurrentUserId();
        var adminRole = GetCurrentUserRole();

        var task = await _taskService.CreateAdminTaskAsync(
            request,
            adminUserId,
            adminRole);

        // Notify the assigned user only.
        if (task.AssignedToUserId.HasValue)
        {
            await _hubContext
                .Clients
                .User(task.AssignedToUserId.Value.ToString())
    ...
    [Authorize(Roles = "Admin")]
    public async Task<IActionResult> UpdateAdminTaskStatus(
        int id,
        [FromBody] UpdateTaskStatusRequest request)
    {
        var adminUserId = GetCurrentUserId();
    ...
    [Authorize(Roles = "Admin")]
    public async Task<IActionResult> UpdateAdminTaskDueDate(
        int id,
    ...
    [Authorize(Roles = "Admin")]
    public async Task<IActionResult> UpdateAdminTaskPriority(
        int id,
    ...
    [Authorize(Roles = "Admin")]
    public async Task<IActionResult> UpdateAdminTask(
        int id,
        [FromBody] UpdateTaskRequest request)
    {
        var adminUserId = GetCurrentUserId();
